...
3. A destination is picked: random POI from Overpass (categorized: nature, food, activity, culture, or "any"), random road point, or fully random point. The candidate must sit closer than half the budget, since the walk is longer than the crow-flies distance out and back — `generate.js` divides by `ROUND_TRIP_SCALE` (2.6), or `ROUND_TRIP_SCALE_WIDE` (3.4) under the toggle in step 4
