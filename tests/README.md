...
The server suite calls `truncateAll()` in `beforeEach`, so it must never touch the prod `wander` DB. `server/tests/test-db.ts` resolves the connection: it derives the DB name from `.env`'s `DATABASE_URL` (or an explicit `TEST_DATABASE_URL`), forces the name to `*_test`, and **hard-throws** unless it ends in `_test` — so a prod-pointing `.env` (Helm copies it into worktrees) can never be truncated. `server/vitest.config.ts` sets `fileParallelism: false` because every DB-backed file shares the one `wander_test` DB and would otherwise race on truncate. That only serializes files inside one vitest process, so **`globalSetup: './tests/global-setup.ts'` serializes the processes**: it holds a Postgres advisory lock on the resolved test database for the length of the run, and releases it after the last file (idea #4042).

That is the half that was missing. Every worktree carries a verbatim copy of `server/.env`, so every session resolves to the same `wander_test`; two sessions truncating each other mid-test produced 27 failed / 340 passed on 2026-09-01 and 367/367 on the immediate retry with no code change. Measured again while building the lock: the same four files run concurrently report 10 and 13 failures without it, 68/68 both with it. A second run now waits (`⏳ wander_test is busy …`) and says how long it waited once it starts.

An advisory lock rather than a lockfile because it is held by the *connection*: a SIGKILLed session drops its socket and Postgres frees the lock, where a stale lockfile would wedge every later run. `scripts/reset-test-db.ts` takes the same lock — dropping the schema under a sibling's running suite is the worse half of this. To skip the wait entirely, point `TEST_DATABASE_URL` at a `*_test` database of your own: the lock is taken on whatever database the run resolves to, so a private one is uncontended by construction. The wait is bounded (10 min) and fails with the `pg_stat_activity` query for finding the holder, rather than hanging.
